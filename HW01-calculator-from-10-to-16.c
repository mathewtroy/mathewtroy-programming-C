...
int main() {
  ...
  int i=1,tmp, soucet, rozdil, soucin, podil;
  float prumer;
  ...
  second = decNum2;
        ...
        for(lenDeciFirst=first; lenDeciFirst>0; lenDeciFirst=lenDeciFirst/16) {
          tmp = lenDeciFirst % 16;
          if( tmp < 10)
          tmp = tmp + 48; else
          tmp = tmp + 55;
          decFirst=decFirst*100+tmp;
              ...
              for(lenDeciSec=second; lenDeciSec>0; lenDeciSec=lenDeciSec/16) {
                tmp = lenDeciSec % 16;
                if( tmp < 10)
                tmp = tmp + 48; else
                tmp = tmp + 55;
